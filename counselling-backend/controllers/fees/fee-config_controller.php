...
if (isset($_GET['get_config'])) {
    ...
    $sql = "SELECT fc.*, 
        ...
        WHERE fc.id = ?";
    $config = $dbOps->customSelect($sql, [$_GET['get_config']]);
...
if (isset($_GET['get_terms'])) {
    header('Content-Type: application/json');
    $terms = $dbOps->select('tbl_term', ['id', 'term_name', 'term_number'], ['academic_year_id' => $_GET['academic_year_id'], 'is_active' => 1], 'term_number ASC');
