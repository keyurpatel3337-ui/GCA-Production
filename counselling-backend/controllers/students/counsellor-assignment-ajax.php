...
$counsellor_id = isset($_POST['counsellor_id']) && $_POST['counsellor_id'] !== '' ? intval($_POST['counsellor_id']) : null;
...
try {
    ...
    if ($action === 'remove' || empty($counsellor_id)) {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Counsellor ' . htmlspecialchars($counsellor['name'] ?? '', ENT_QUOTES, 'UTF-8') . ' assigned successfully'
        ]);
    }
