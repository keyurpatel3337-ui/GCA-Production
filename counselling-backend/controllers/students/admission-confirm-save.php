...
$scholarship_rule_id = isset($_POST['scholarship_rule_id']) && $_POST['scholarship_rule_id'] !== '' ? intval($_POST['scholarship_rule_id']) : null;
$scholarship_percentage = floatval($_POST['scholarship_percentage'] ?? 0);
$scholarship_amount = floatval($_POST['scholarship_amount'] ?? 0);
$additional_scholarship_type = preg_replace('/[^a-zA-Z0-9_-]/', '', $_POST['additional_scholarship_type'] ?? null);
$additional_scholarship_value = floatval($_POST['additional_scholarship_value'] ?? 0);
$additional_scholarship_amount = floatval($_POST['additional_scholarship_amount'] ?? 0);
$additional_scholarship_remarks = trim($_POST['additional_scholarship_remarks'] ?? '');
$payment_mode = preg_replace('/[^a-zA-Z0-9_-]/', '', $_POST['payment_mode'] ?? 'offline');
$remarks = trim($_POST['remarks'] ?? '');
$hostel_required = preg_replace('/[^a-zA-Z0-9_-]/', '', $_POST['hostel_required'] ?? 'No');
$transport_required = preg_replace('/[^a-zA-Z0-9_-]/', '', $_POST['transport_required'] ?? 'No');
$counsellor_id = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : null;
...
try {
    ...
    if ($is_api_call) {
        sendErrorResponse('Error confirming admission: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8'), 500);
    }
