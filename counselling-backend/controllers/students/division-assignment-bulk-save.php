...
try {
    $course_division_id = $_POST['course_division_id'] ?? null;
    $student_ids = $_POST['student_ids'] ?? [];

    if (empty($course_division_id) || empty($student_ids)) {
    ...
    foreach ($student_ids as $enrollment_id) {
        try {
            ...
            if ($duplicate_check) {
                $failed_assignments[] = "{$student['surname']} {$student['student_name']} (Reg ID: {$student['registration_id']}) - Duplicate enrollment exists in this division";
                continue;
            }

            if ($student['course_id'] != $course_division['course_id'] || $student['group_id'] != $course_division['group_id']) {
                $failed_assignments[] = "{$student['surname']} {$student['student_name']} - Course/Group mismatch";
                continue;
            }

            // Check if student is already assigned to this division
            if ($student['division_id'] == $division_id) {
                $failed_assignments[] = "{$student['surname']} {$student['student_name']} - Already assigned to this division";
                continue;
            }
            ...
            if ($student['division_id'] && $student['roll_no']) {
                $failed_assignments[] = "{$student['surname']} {$student['student_name']} - Already assigned to Division {$student['division_id']} with Roll No {$student['roll_no']}";
                continue;
            }
