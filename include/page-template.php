...
function getPage($slug)
{
    ...
    try {
        return $dbOps->selectOne('tbl_pages', ['*'], [
            'page_slug' => $slug,
            'is_active' => 1
        ]);
    } catch (Exception $e) {
...
function getPageSections($pageId)
{
    ...
    try {
        return $dbOps->select('tbl_page_sections', ['*'], [
            'page_id' => $pageId,
            'is_active' => 1
        ], 'display_order ASC') ?: [];
    } catch (Exception $e) {
...
function getPageFeatures($pageId, $sectionId = null)
{
    ...
    try {
        ...
        return $dbOps->select('tbl_page_features', ['*'], $where, 'display_order ASC') ?: [];
    } catch (Exception $e) {
...
function getPageTeam($pageId)
{
    ...
    try {
        return $dbOps->select('tbl_page_team', ['*'], [
            'page_id' => $pageId,
            'is_active' => 1
        ], 'display_order ASC') ?: [];
    } catch (Exception $e) {
