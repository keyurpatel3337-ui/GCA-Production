<?php
/**
 * Migration: Add created_by columns to all tables where needed (Fixed Version)
 * Date: 2026-01-18
...
echo "<h2>Migration: Adding created_by Columns (Fixed)</h2>";
...
function addColumn($conn, $table, $column, $definition, $after = null)
{
    ...
    if (!tableExists($conn, $table)) {
        echo "⚠️ <span style='color: orange;'>SKIP:</span> Table <b>$table</b> does not exist<br>";
        $skipped_count++;
        return false;
    }
...
function addIndex($conn, $table, $indexName, $column)
{
    try {
        ...
        echo "✅ <span style='color: green;'>INDEX:</span> Added index <b>$indexName</b> on <b>$table.$column</b><br>";
        return true;
    } catch (Exception $e) {
        return false;
    }
...
try {
    // Note: MySQL auto-commits DDL statements, so we can't use transactions
    ...
    addIndex($conn, 'tbl_installment_requests', 'idx_created_by', 'created_by');
    addIndex($conn, 'tbl_installment_requests', 'idx_request_type', 'request_type');
    ...
    // 8. tbl_pending_payments - check first if exists
