...
$response = $apiClient->get('settings/groups');
...
<link rel="stylesheet" href="<?php echo PORTAL_URL; ?>/assets/css/modules/academics/group.css">

<div class="container-fluid py-4">
    <div class="card shadow-sm overflow-hidden border-0">
        <div class="card-header bg-white py-3 px-4 border-bottom d-flex align-items-center justify-content-between">
            <h5 class="card-title mb-0 fw-bold text-dark">
                <i class="fas fa-layer-group me-2 text-primary"></i> Group List
            </h5>
            <div class="d-flex align-items-center gap-2">
                ...
                <div id="deleteSelectedBtn" class="group-custom-1">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="group_name" class="form-control"
                            placeholder="e.g., A Group (Board), B Group (Board/NEET)" required>
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="editModal" tabindex="-1">
    ...
    <script>
        ...
        function exportToExcel() {
            TableUtils.exportToExcel('groupsTable', 'groups_export');
        }
        ...
        function deleteSelected() {
            ...
            if (selectedIds.length === 0) {
                showToast('warning', 'Warning', 'Please select at least one group to delete!');
                return;
            }
            showConfirm({
                title: 'Delete Selected Groups?',
                message: `You are about to delete ${selectedIds.length} group(s)!`,
                confirmText: 'Yes, delete them!',
                confirmButtonClass: 'btn-danger',
                onConfirm: function () {
                    $.api.post('settings/group-delete-multiple', {
                        ids: selectedIds
                    }).then(response => {
                        if (response.success) {
        ...
        $('#addForm').on('submit', function (e) {
            e.preventDefault();
            $.api.post('settings/group-save', $(this).serialize()).then(response => {
                if (response.success) {
        ...
        $('#editForm').on('submit', function (e) {
            e.preventDefault();
            $.api.post('settings/group-update', $(this).serialize()).then(response => {
                if (response.success) {
                    showToast('success', 'Success', response.message);
                    setTimeout(() => location.reload(), 1500);
                } else {
        ...
        function deleteItem(id) {
            showConfirm({
                ...
                confirmButtonClass: 'btn-danger',
                onConfirm: function () {
                    $.api.post('settings/group-delete', {
                        id: id
                    }).then(response => {
                        if (response.success) {
                            showToast('success', 'Deleted!', response.message);
