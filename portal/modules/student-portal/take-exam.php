...
if (!$exam['attempt_id']) {
    ...
    if ($attempt_info) {
        ...
        if ($attempt_info['last_active_timestamp'] !== null && $inactive_mins >= 30) {
            ...
            die("<div style='font-family: sans-serif; text-align: center; margin-top: 100px;'><h1 style='color: #ef4444;'>Session Expired</h1><p style='color: #64748b;'>This exam session has been automatically finalized due to exceeding the 30-minute inactivity limit.</p><a href='online-exams.php' style='display: inline-block; margin-top: 20px; padding: 10px 20px; background-color: #0d9488; color: white; text-decoration: none; border-radius: 5px;'>Back to Dashboard</a></div>");
        }
...
if ($remaining_seconds <= 0) {
    $sub_stmt = $conn->prepare("UPDATE tbl_oes_student_exams SET status = 'Submitted', submit_timestamp = NOW() WHERE id = ?");
    $sub_stmt->execute([$attempt_id]);
    die("<div style='font-family: sans-serif; text-align: center; margin-top: 100px;'><h1 style='color: #ef4444;'>Exam Time Over</h1><p style='color: #64748b;'>The total allowed duration for this exam has already elapsed.</p><a href='online-exams.php' style='display: inline-block; margin-top: 20px; padding: 10px 20px; background-color: #0d9488; color: white; text-decoration: none; border-radius: 5px;'>Back to Dashboard</a></div>");
}
...
<head>
    ...
    <title>Practice Exam Terminal | GCA</title>
...
<body class="exam-body-secure">
    ...
    <script>
        ...
        function initProctoredListeners() {
            ...
            // 4. Prevent keyboard shortcuts
            window.addEventListener('keydown', (e) => {
                if (
                    e.key === 'F12' || 
                    (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'J' || e.key === 'C')) ||
                    (e.ctrlKey && (e.key === 'u' || e.key === 'U' || e.key === 'c' || e.key === 'C' || e.key === 'v' || e.key === 'V' || e.key === 'a' || e.key === 'A' || e.key === 's' || e.key === 'S')) ||
                    ...
                    Swal.fire({
                        ...
                        text: 'Security Alert: Right-clicks, page copies, and developer console triggers are deactivated under Proctored Mode!',
                        confirmButtonColor: '#0d9488'
                    });
                    return false;
                }
        ...
        function loadQuestion(idx) {
            ...
            // Handle Quill formula spans specifically
        ...
        function confirmFinish() {
            ...
            Swal.fire({
                title: 'Finish & Submit?',
                text: 'Are you sure you want to finish and submit the exam? You cannot modify your answers after this.',
                icon: 'question',
        ...
        async function submitExam() {
            ...
            window.location.href = 'online-exams.php';
        }
        ...
        function autoSubmit() {
            Swal.fire({
                icon: 'warning',
                title: 'Time Is Up!',
                text: 'Time is up! Your exam is being submitted automatically.',
                confirmButtonColor: '#0d9488',
                allowOutsideClick: false,
                allowEscapeKey: false
            }).then(() => {
