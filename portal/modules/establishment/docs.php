...
try {
    ...
    if ($student_id) {
        ...
        if ($student_data) {
            $stmt = $conn->prepare("SELECT d.*, u.name as uploader_name FROM tbl_student_documents d LEFT JOIN tbl_users u ON d.uploaded_by = u.id WHERE d.student_id = ? ORDER BY d.uploaded_at DESC");
            $stmt->execute([$student_id]);
