...
if (isset($_GET['export']) && $_GET['export'] == 'excel') {
    ...
    header('Content-Disposition: attachment; filename="hostel_fees_' . date('Y-m-d') . '.xlsx"');
...
// Total Count
$countQuery = "SELECT COUNT(*) as total FROM ($baseQuery) as count_tbl";
$countResult = $dbOps->customSelect($countQuery, $params);
$totalRecords = $countResult[0]['total'] ?? 0;
$totalPages = ceil($totalRecords / $limit);

// Total Collected Amount
$sumQuery = "SELECT SUM(amount) as total_collected FROM ($baseQuery) as sum_tbl";
$sumResult = $dbOps->customSelect($sumQuery, $params);
$totalCollected = $sumResult[0]['total_collected'] ?? 0;

// Fetch Data
$sql = "$baseQuery ORDER BY p.payment_date ASC LIMIT $limit OFFSET $offset";
$payments = $dbOps->customSelect($sql, $params);

// Counts for individual components within the same filtered base
$hostelFeeCount = 0;
$securityDepositCount = 0;

$hfQuery = "SELECT COUNT(*) as total FROM ($baseQuery) as hf_tbl WHERE fee_component = 'hostel_fee'";
$hfResult = $dbOps->customSelect($hfQuery, $params);
$hostelFeeCount = $hfResult[0]['total'] ?? 0;

$sdQuery = "SELECT COUNT(*) as total FROM ($baseQuery) as sd_tbl WHERE fee_component = 'hostel_security'";
$sdResult = $dbOps->customSelect($sdQuery, $params);
$securityDepositCount = $sdResult[0]['total'] ?? 0;
...
<div class="container-fluid">
            ...
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <select name="course_id" class="form-select">
                        ...
                        <option value="Reneet" <?php echo ($course_filter == 'Reneet') ? 'selected' : ''; ?>>Reneet
                        </option>
                    </select>
                </div>
                <div class="col-md-2">
                    ...
                    <select name="fee_type" class="form-select">
                        ...
                        <option value="hostel_fee" <?php echo ($fee_type == 'hostel_fee') ? 'selected' : ''; ?>>Hostel Fee
                        </option>
                        <option value="hostel_security" <?php echo ($fee_type == 'hostel_security') ? 'selected' : ''; ?>>
                ...
                <div class="col-md-3 d-flex align-items-end gap-1">
                    ...
                    <a href="hostel-fees.php" class="btn btn-secondary btn-sm"><i class="fas fa-redo"></i> Reset</a>
    ...
    <div class="card">
                        ...
                        <?php else: ?>
                            ...
                            foreach ($payments as $p): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge <?php echo $p['fee_component'] == 'hostel_security' ? 'bg-warning' : 'bg-primary'; ?>">
                                            <?php echo $p['fee_component'] == 'hostel_security' ? 'Security Deposit' : 'Hostel Fee'; ?>
                                        </span>
                                    </td>
        ...
        <div class="card-footer clearfix">
            <?php
            $baseUrl = "hostel-fees.php?" . http_build_query(array_diff_key($_GET, ['page' => '']));
            echo renderPagination($page, $totalPages, $baseUrl, 2, $totalRecords, 'hostel fee payments');
            ?>
        </div>
    </div>
</div>
...
<script>
    function exportToPDF() {
        ...
        window.location.href = 'hostel-fees-pdf.php?' + params.toString();
    }
</script>
