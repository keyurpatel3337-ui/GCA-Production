...
if (!isset($_SESSION['user_id']) && !isset($_SESSION['student_id']) && !isset($_SESSION['is_parent_login'])) {
    // Dynamically calculate path to index.php based on current location
    $depth = substr_count(str_replace('\\', '/', dirname($_SERVER['SCRIPT_FILENAME'])), '/') - substr_count(str_replace('\\', '/', dirname(__DIR__)), '/');
    // Ensure depth is not negative (for scripts in portal root)
    $depth = max(0, $depth);
    $redirect = str_repeat('../', $depth) . 'index.php';
...
try {
    ...
    if (!isset($conn)) {
        require_once dirname(__DIR__) . '/../common/db_connect.php';
    }

    // Only query if connection is available
...
if (!defined('SYSTEM_EMAIL')) {
    define('SYSTEM_EMAIL', $system_settings['contact_email'] ?? 'info@counsellingportal.com');
}
if (!defined('ORGANIZATION_NAME')) {
    define('ORGANIZATION_NAME', $system_settings['organization_name'] ?? 'Gyanmanjari Career Academy');
}
...
if (!defined('ROLE_GROCERY_MANAGER')) {
    define('ROLE_GROCERY_MANAGER', 10);
}
...
// Session Variables
// Support both regular users, student-specific login, and parent login
if (isset($_SESSION['is_student_login']) && $_SESSION['is_student_login'] === true) {
    ...
    $student_id = $_SESSION['active_student_id'] ?? null; // Explicitly set $student_id
    $user_name = $_SESSION['parent_mobile'] ?? 'Parent';
    $user_email = '';
    $user_role = 'parent';
    $role_name = 'Parent';
    $role_id = 0; // Parents don't have a numeric role ID in tbl_roles
} else {
