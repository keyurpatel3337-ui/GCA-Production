...
$request_id = isset($_POST['request_id']) ? intval(htmlspecialchars($_POST['request_id'], ENT_QUOTES, 'UTF-8')) : null;
$action = htmlspecialchars(preg_replace('/[^a-zA-Z0-9_-]/', '', $_POST['action'] ?? 'show_form'), ENT_QUOTES, 'UTF-8');

if (!$request_id || $request_id <= 0) {
    echo json_encode(['success' => false, 'message' => 'Request ID is required'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
    exit;
}
...
try {
    ...
    if (!$request) {
        echo json_encode(['success' => false, 'message' => 'Request not found'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
        exit;
    }

    // Check if request is approved
    if ($request['request_status'] !== 'approved') {
        echo json_encode(['success' => false, 'message' => 'Only approved requests can be processed'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
        exit;
    }

    // Check if refund mode is gateway
    if ($request['refund_mode'] !== 'gateway') {
        echo json_encode(['success' => false, 'message' => 'This request uses ' . htmlspecialchars($request['refund_mode'] ?? '', ENT_QUOTES, 'UTF-8') . ' mode, not gateway'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
        exit;
    }
    ...
    if ($action === 'show_form') {
        ...
        echo json_encode(['success' => true, 'html' => $html], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
    ...
    echo json_encode(['success' => false, 'message' => 'Error: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8')], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
}
...
function generateProcessingForm($request)
{
    ...
    $html .= '<tr><th>Original Amount:</th><td><strong>₹' . htmlspecialchars(formatIndianCurrency($request['payment_amount']), ENT_QUOTES, 'UTF-8') . '</strong></td></tr>';
    $html .= '<tr><th>Refund Amount:</th><td><strong class="text-danger">₹' . htmlspecialchars(formatIndianCurrency($request['refund_amount']), ENT_QUOTES, 'UTF-8') . '</strong></td></tr>';
    $html .= '</table>';
    $html .= '</div>';
    ...
    $html .= '<tr><th>Payment Date:</th><td>' . htmlspecialchars(date('d-M-Y', strtotime($request['payment_date'])), ENT_QUOTES, 'UTF-8') . '</td></tr>';
    $html .= '<tr><th>Payment Mode:</th><td>' . htmlspecialchars(ucfirst($request['payment_mode']), ENT_QUOTES, 'UTF-8') . '</td></tr>';
    ...
    $html .= '<button type="button" class="btn btn-primary" onclick="confirmProcessRefund(' . intval($request['id']) . ')">';
